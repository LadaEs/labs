...
function init() {
    ...
    window.addEventListener('resize',onWindowRelize, false);
    
}

function onWindowRelize() {
...
function render() {
    if(isLoaded == false && !!plane) { //!! - проверка на false и underfined
        isLoaded = true;
        scene.add(plane);
    }
    renderer.render(scene,camera);
